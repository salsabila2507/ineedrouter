...
import { rankModelsForStrategy, recordModelLatency } from "./routingIntelligence.js";
 ...
 * @param {string} [options.comboStrategy] - Strategy: fallback, round-robin, or an automatic routing mode
...
export async function handleComboChat({ body, models, handleSingleModel, log, comboName, comboStrategy, comboStickyLimit = 1, autoSwitch = true }) {
  ...
  // Automatic modes only rank models already selected by the operator for this combo.
  // Unknown metrics remain neutral, preserving stable operator order on ties.
  const rankedModels = rankModelsForStrategy(rotatedModels, comboStrategy);
  if (rankedModels[0] !== rotatedModels[0]) {
    log.info("COMBO", `${comboStrategy} routing -> ${rankedModels[0]}`);
  }
  rotatedModels = rankedModels;
  ...
  for (let i = 0; i < rotatedModels.length; i++) {
    ...
    try {
      ...
      try {
        ...
        // Measures upstream response/setup latency (including time-to-headers for streams),
        // not full streamed completion time.
        recordModelLatency(modelStr, Date.now() - startedAt);
      }
