...
export default function DroidToolCard({
  ...
  const getConfigStatus = () => {
    ...
    // Check for any iNeedRouter model entry (support multi-model: custom:iNeedRouter-0, custom:iNeedRouter-1, ...)
    const currentConfig = droidStatus.settings?.customModels?.find(m => (m.id?.startsWith("custom:iNeedRouter") || m.id?.startsWith("custom:9Router")));
  ...
  useEffect(() => {
    if (droidStatus?.installed && !hasInitializedModel.current) {
      ...
      const existingModels = (droidStatus.settings?.customModels || [])
        .filter(m => (m.id?.startsWith("custom:iNeedRouter") || m.id?.startsWith("custom:9Router")))
        .sort((a, b) => (a.index || 0) - (b.index || 0))
        .map(m => m.model);
      if (existingModels.length > 0) {
        setModelList(existingModels);
      } else {
        // Legacy: single model stored as custom:iNeedRouter-0
        const legacy = droidStatus.settings?.customModels?.find(m => (m.id === "custom:iNeedRouter-0" || m.id === "custom:9Router-0"));
        if (legacy?.model) {
              ...
              <div className="flex flex-col gap-3 p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-text-muted">Manual configuration is still available if iNeedRouter is deployed on a remote server.</p>
                  </div>
                </div>
          ...
          {!checkingDroid && droidStatus?.installed && (
            <>
              <div className="flex flex-col gap-2">
                ...
                {/* Current configured */}
                {droidStatus?.settings?.customModels?.find(m => (m.id?.startsWith("custom:iNeedRouter") || m.id?.startsWith("custom:9Router")))?.baseUrl && (
