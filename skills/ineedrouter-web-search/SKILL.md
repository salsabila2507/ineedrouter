---
name: web-search
description: Web search via iNeedRouter /v1/search using Tavily / Exa / Brave / Serper / SearXNG / Google PSE / Linkup / SearchAPI / You.com / Perplexity. Use when the user wants to search the web, look up information, find articles, or query a search engine.
---

# iNeedRouter — Web Search
...
```bash
curl $INEEDROUTER_URL/v1/models/web | jq '.data[] | select(.kind=="webSearch") | .id'
# Per-provider params (searchTypes, maxResults, required options like cx for google-pse)
curl "$INEEDROUTER_URL/v1/models/info?id=tavily/search"
```
...
`POST $INEEDROUTER_URL/v1/search`
...
```bash
curl -X POST $INEEDROUTER_URL/v1/search \
  -H "Authorization: Bearer $INEEDROUTER_KEY" \
  -H "Content-Type: application/json" \
  -d '{"model":"tavily","query":"iNeedRouter open source","max_results":5}'
```

JS:

```js
const r = await fetch(`${process.env.INEEDROUTER_URL}/v1/search`, {
  method: "POST",
