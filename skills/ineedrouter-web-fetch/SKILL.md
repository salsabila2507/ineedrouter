---
name: web-fetch
description: Fetch URL → markdown / text / HTML via iNeedRouter /v1/web/fetch using Firecrawl / Jina Reader / Tavily Extract / Exa Contents. Use when the user wants to scrape a webpage, extract URL content, read article, or convert a URL to markdown.
---

# iNeedRouter — Web Fetch
...
```bash
curl $INEEDROUTER_URL/v1/models/web | jq '.data[] | select(.kind=="webFetch") | .id'
# Per-provider params
curl "$INEEDROUTER_URL/v1/models/info?id=firecrawl/fetch"
```
...
`POST $INEEDROUTER_URL/v1/web/fetch`
...
### Jina Reader
```bash
curl -X POST $INEEDROUTER_URL/v1/web/fetch \
  -H "Authorization: Bearer $INEEDROUTER_KEY" \
  -H "Content-Type: application/json" \
  -d '{"model":"jina-reader","url":"https://iNeedRouter.com","format":"markdown"}'
```
...
```js
const r = await fetch(`${process.env.INEEDROUTER_URL}/v1/web/fetch`, {
  method: "POST",
