...
Triển khai iNeedRouter trên VPS hoặc Docker để truy cập từ xa và dùng trong production.
...
| `DATA_DIR` | `~/.ineedrouter` | Đường dẫn lưu database và data |
...
Tạo `/etc/nginx/sites-available/ineedrouter`:
...
# If NOT using reverse proxy, allow iNeedRouter ports
...
```bash
# Check if iNeedRouter is running
pm2 status
