...
Tích hợp iNeedRouter với OpenAI Codex CLI để định tuyến request API OpenAI qua hệ thống routing thông minh của iNeedRouter.
...
iNeedRouter cung cấp các model Codex sau:
...
```json
{
  ...
  "apiKey": "your-ineedrouter-api-key",
...
1. Xác minh API key đúng trong iNeedRouter dashboard
...
1. Xác minh tên model khớp với cấu hình iNeedRouter
2. Kiểm tra kết nối provider OpenAI đang hoạt động trong iNeedRouter dashboard
...
```bash
export OPENAI_BASE_URL="https://ineed.web.id"
```

Đảm bảo bạn đã cấu hình API key trong iNeedRouter cloud dashboard.
