...
Guía detallada de instalación de iNeedRouter con consejos de solución de problemas.
...
Instala iNeedRouter globalmente para usar desde cualquier lugar:
...
- ✅ Comando simple: `ineedrouter`
- ✅ Auto-actualizaciones con `npm update -g ineedrouter`
...
```bash
mkdir my-ineedrouter
cd my-ineedrouter
npm install ineedrouter
```
...
3. Se crea el directorio de datos en `~/.ineedrouter`
...
**Ubicación por defecto:** `~/.ineedrouter`
...
**Error:**
```
Error: EACCES: permission denied, mkdir '/usr/local/lib/node_modules/ineedrouter'
```
...
**Problema:** iNeedRouter usa demasiada RAM
...
```bash
npm install -g pm2
pm2 start ineedrouter --name ineedrouter
pm2 save
```
...
```bash
docker pull ineedrouter/ineedrouter:latest

docker run -d \
  -p 20128:20128 \
  -e JWT_SECRET="your-secure-secret" \
  -e INITIAL_PASSWORD="your-password" \
  -v ineedrouter-data:/root/.ineedrouter \
  --name ineedrouter \
  ineedrouter/ineedrouter:latest
```
...
nano ~/.bashrc  # o ~/.zshrc
# Eliminar exports relacionados con ineedrouter
```
...
- **Sitio web**: [ineed.web.id](https://ineed.web.id)
