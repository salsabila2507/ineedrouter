...
リモートアクセスと本番利用のため、VPSまたはDockerにiNeedRouterをデプロイ。
...
| `DATA_DIR` | `~/.ineedrouter` | データベースとデータの保存パス |
...
`/etc/nginx/sites-available/ineedrouter` を作成:
...
# リバースプロキシを使用しない場合、iNeedRouterポートを許可
